...
<task type="auto">
  <name>Task 2: Update OAuth callback to issue mcp_token and add /mcp/:userToken Streamable HTTP route</name>
  <files>src/server.ts</files>
  <action>
This task has two parts: (A) update `/oauth/callback` to generate and return the user's personal MCP URL, and (B) add the full `/mcp/:userToken` POST/GET/DELETE route with session Map lifecycle.

**Part A — Update `/oauth/callback` to issue mcp_token:**

In `src/server.ts`, update the `/oauth/callback` handler to:
1. After `tokenStore.save(tokenRecord)`, generate a UUID using `crypto.randomUUID()` (Node built-in, no import needed in ESM)
2. Call `tokenStore.saveMcpToken(tokenRecord.basecampUserId, mcpToken)` to persist it
3. Derive the base URL from `BASECAMP_REDIRECT_URI` env var or default to `http://localhost:3000`
4. Return the personal MCP URL in the JSON response so the user can copy-paste it into Claude Desktop/Cursor

Updated callback handler (replace the entire `app.get('/oauth/callback', ...)` block):
...
**Part B — Add session Map and /mcp/:userToken route:**

Add the following to `src/server.ts` after the `/oauth/revoke` route and before the `/health` route.

First, add these imports at the top of `src/server.ts` (after the existing imports):
...
Then add the sessions Map type and the route (place this block after the `/oauth/revoke` handler):
...
Run `npx tsc --noEmit`. If `StreamableHTTPServerTransport` is not found at the import path, check the SDK version and try `@modelcontextprotocol/sdk/server/streamableHttp.js` (lowercase h). The correct path in SDK ^1.15.0 is `@modelcontextprotocol/sdk/server/streamableHttp.js`.

If `crypto.randomUUID` raises a type error, add `/// <reference lib="dom" />` or use `import { randomUUID } from 'node:crypto'` and call `randomUUID()`.
  </action>
  <verify>
1. `npx tsc --noEmit` exits 0
2. `grep -n "StreamableHTTPServerTransport\|sessions\|onsessioninitialized\|getByMcpToken" src/server.ts | head -20` shows all four
3. `grep "mcp_url" src/server.ts` shows the mcp_url field in the /oauth/callback response
4. `grep "crypto.randomUUID\|randomUUID" src/server.ts | head -5` shows UUID generation in both callback and sessionIdGenerator
5. `grep "/mcp/:userToken" src/server.ts` shows the route registration
  </verify>
  <done>
src/server.ts: /oauth/callback generates and returns mcp_url. /mcp/:userToken handles POST/GET/DELETE via app.all(). Sessions Map binds sessionId to { transport, server, userId }. Unknown userToken returns 401. transport.onclose cleans up sessions Map. tsc compiles clean.
  </done>
</task>

<task type="auto">
  <name>Task 3: Create src/index.ts entry point with TRANSPORT=stdio conditional</name>
